...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    if (isset($_GET['user_id'])) {
        ...
        $stmt = $conn->prepare("SELECT * FROM checkout WHERE user_id = :user_id AND status_pengiriman = 'Dikirim'");
        ...
        if ($stmt->rowCount() > 0) {
            ...
            echo json_encode([
                'success' => false,
                'message' => 'No Checkout Data Found for the given user_id and status_pengiriman "Dikirim"!',
            ]);
        }
    } else {
        echo json_encode([
